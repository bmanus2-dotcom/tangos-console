...
const WORK_GUIDE_DEFAULT =
  ...
  '10. If this batch includes MATCH LOGGING rules below, emit one MATCH_RESULT node per target for every try (including no_progress). Copy SHARED DEFAULTS once - do not re-invent provenance fields per function. matched only after verify MATCH; near_miss only when the tip improves; same-or-worse score = no_progress; never log wall-clock times; bank is not a new try.\n' +
